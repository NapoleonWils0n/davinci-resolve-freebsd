...
change username to your username on freebsd
...
copy the ubuntu script to /usr/local/etc/rc.d/ in the chroot
...
create the /compat/ubuntu directory and sub directories
...
if you have install centos using the linuxulator \\
and it is installed in /compat/linux 
...
because you can only run linuxulator install at a time
