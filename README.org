...
** Xwayland start Davinci Resolve

Starting Davinci Resolve manually on Wayland running on Freebsd
...
make sure you have switched to your user and aren't running as root
...
+ QT_QPA_PLATFORM is set to xcb, Davinci Resolve is use the qt framework but doesnt run as a native wayland application
